...
mtcars_piped = mtcars %>% filter(hp > 100 & gear == 4) %>% select(mpg, cyl)


mtcars_2 $ sutun = mtcars_2 $ gear*10
view(mtcars_2)


mtcars_mutate = mutate(mtcars, newcol = gear*10)
view(mtcars_mutate)


mtcars_ifelse_mutate = mutate(mtcars,
                              disp_cat = ifelse(
                                disp<=200,
                                "düşük",
                                ifelse(disp<=400,
...
view(mtcars_ifelse_mutate)


disp_siralanmıs <- arrange(mtcars_ifelse_mutate, desc(disp))


help("transmute")
??transmute
mtcars_transmute = transmute(mtcars, yenisutun = disp*10, mpg, gear)
